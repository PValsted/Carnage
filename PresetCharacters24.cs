...
namespace Carnage
{
    ...
    public partial class PresetCharacters24 : Form
    {
        ...
        public PresetCharacters24(Game game, VisualSettings vs)
        {
            InitializeComponent();
            this.game = game;
            this.vs = vs;
        ...
        private void btnEnterCharacters_Click(object sender, EventArgs e)
        {
            ...
            {
                rtbSearch.Clear();
                if (game.Mode == "Classic") charList = ci.ConvertToCharacters(stringList, "Classic", 24, game); //If classic: pronouns are paired with the names to create 24 characters
                else if (game.Mode == "Realistic") charList = ci.ConvertToCharacters(stringList, "Realistic", 24, game); //If realistic: pronouns, speed, strength, and morality are paired with the names to create 24 characters
